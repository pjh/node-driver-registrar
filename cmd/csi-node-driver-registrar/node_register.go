...
func nodeRegister(
	...
	if err != nil {
		klog.Warningf("pjh: Listen failed with error %s", err.Error())
		...
		if strings.Contains(err.Error(), "Only one usage of each socket address") {
			klog.Warningf("pjh: removing %s and listening again", socketPath)
	...
	if err := grpcServer.Serve(lis); err != nil {
		...
		klog.Warningf("pjh: deleting socket %s before exit", socketPath)
		_ = os.Remove(socketPath)
		os.Exit(1)
	}
	klog.Warningf("pjh: gRPC server shut down, deleting socket %s before exit", socketPath)
	_ = os.Remove(socketPath)
